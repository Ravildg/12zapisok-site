import React, { useState, useEffect } from "react";// Структура для игры
interface Game {
  title: string;
  description: string;
  players: string;
...
const initialGamesData: Game[] = [
  ...
  const [gamesData, setGamesData] = useState<Game[]>(initialGamesData);  // Сохраняем игры в localStorage
  useEffect(() => {
...
}Перепиши этот  код без ошибок что бы все сохранялось на сайте при нажатии кнопки сохранить, что бы текст в поллях соответсвовал тому что на сайте, что бы колличество игр соответсвовало тому что есть на сатйте, в коде все оэто есть
